...
public class ChinaMobileRemoteExecute {
	...
	public static Result login(String key, String login, String pwd) {
		...
		try {
			...
			if (!"0000".equals(data.get("code"))) {
				res = new Result(Constants.INPUTERROR, data.get("desc"));
			} else {
				res = new Result(Constants.SUCCESS, data.get("desc"));
			}
	...
	public static Result auth(String key, String servPwd, String smsPwd, String imgCode, boolean isSMS) {
		String sessionId = SessionUtils.getSessionId(key);
		if (sessionId == null) {
			return new Result(Constants.SYSTEMERROR, Constants.getMessage(Constants.SYSTEMERROR));
		}
		...
		String jsEnd = "if($.isFunction(window.myCb)){window.myCb.apply(window.myContext, window.myData); };delete window.ajaxBack;delete window.myCb";
