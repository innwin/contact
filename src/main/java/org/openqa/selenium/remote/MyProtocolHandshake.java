...
public class MyProtocolHandshake {
	...
	private final static Logger LOG = Logger.getLogger(MyProtocolHandshake.class.getName());
	...
	public Result connSession(HttpClient client, SessionId id) throws IOException {
		...
		HttpRequest request = new HttpRequest(HttpMethod.GET, "/session/" + id);
		...
		if (response.getStatus() == HttpURLConnection.HTTP_OK) {
			if (sessionId != null && capabilities != null) {
				Dialect dialect = ossStatus == null ? Dialect.W3C : Dialect.OSS;
				return Optional.of(new Result(dialect, String.valueOf(sessionId), capabilities)).get();
			}
		}
	...
	private void streamW3CProtocolParameters(JsonWriter out, Gson gson, JsonObject des, JsonObject req)
		...
		Map<String, ?> firefox = Stream.of(des, req).map(JsonObject::entrySet).flatMap(Collection::stream).filter(
				entry -> ("browserName".equals(entry.getKey()) && FIREFOX.equals(entry.getValue().getAsString()))
						|| entry.getKey().startsWith("firefox_") || entry.getKey().startsWith("moz:"))
				.distinct().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (left, right) -> right));
		...
		Map<String, ?> opera = Stream.of(des, req).map(JsonObject::entrySet).flatMap(Collection::stream).filter(
				entry -> ("browserName".equals(entry.getKey()) && OPERA_BLINK.equals(entry.getValue().getAsString()))
